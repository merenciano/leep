...
#define IM_NEW(X) (X*)malloc(sizeof(X))
...
struct ImGui_ImplGlfw_Data
{
	...
	double time = 0.0;
	GLFWwindow *mouse_window = NULL;
	GLFWcursor *mouse_cursors[ImGuiMouseCursor_COUNT] = {0};
	ImVec2 last_valid_mouse_pos = {0.0f, 0.0f};
	GLFWwindow *key_owner_windows[GLFW_KEY_LAST] = {0};
	bool installed_callbacks = false;
	bool want_update_monitors = false;

	GLFWwindowfocusfun prev_window_focus = NULL;
	GLFWcursorposfun prev_cursor_pos = NULL;
	GLFWcursorenterfun prev_cursor_enter = NULL;
	GLFWmousebuttonfun prev_mousebutton = NULL;
	GLFWscrollfun prev_scroll = NULL;
	GLFWkeyfun prev_key = NULL;
	GLFWcharfun prev_char = NULL;
	GLFWmonitorfun prev_monitor = NULL;
};
...
void ImGui_ImplGlfw_CursorPosCallback(GLFWwindow* window, double x, double y)
{
    ...
    bd->last_valid_mouse_pos = {(float)x, (float)y};
}
    ...
    for (int n = 0; n < platform_io->Viewports.Size; n++)
    {
        ...
        if (is_window_focused)
        {
            ...
            if (bd->mouse_window == NULL)
            {
                ...
                bd->last_valid_mouse_pos = {(float)mouse_x, (float)mouse_y};
