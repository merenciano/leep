...
{
	igSetNextWindowPos({10, 10}, ImGuiCond_FirstUseEver, {0, 0});
...
static void THE_UIToolsLuaCommands()
{
	...
	igSetNextWindowPos({240, 10}, ImGuiCond_FirstUseEver, {0, 0});
	igSetNextWindowSize({300, 80}, ImGuiCond_FirstUseEver);
...
{
	igSetNextWindowPos({10, 150}, ImGuiCond_FirstUseEver, {0, 0});
...
static void THE_UIToolsComponentInspector()
{
	...
	igSetNextWindowPos({185, 150}, ImGuiCond_FirstUseEver, {0, 0});
...
static void THE_UIToolsResourceInspector()
{
	...
	igSetNextWindowPos({1050, 10}, ImGuiCond_FirstUseEver, {0, 0});
...
static void THE_UIToolsMemoryUsage()
{
	static ImGuiTableFlags flags = ImGuiTableFlags_ScrollY | 
		...
		ImGuiTableFlags_BordersV | ImGuiTableFlags_Resizable |
		ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable;
	igSetNextWindowPos({10, 560}, ImGuiCond_FirstUseEver, {0, 0});
	...
	if (igBeginTable("##table1", 4, flags, size, 0.0f)) {
		...
		igText("%s", "Materials");
		igTableSetColumnIndex(1);
		igText("%.2f B", (float)kMatPoolSize);
		igTableSetColumnIndex(2);
		igText("%.2f B", (float)kMatPoolSize);
		igTableSetColumnIndex(3);
		igText("%.1f %%", 100.0f);*/
		...
		/* Render queue
		{
		    ...
		    igTableNextRow();
		    igTableSetColumnIndex(0);
		    igText("%s", "Render queue");
		    igTableSetColumnIndex(1);
		    igText("%.2f KB", ByteToKilo(offset));
		    igTableSetColumnIndex(2);
		    igText("%.2f KB", ByteToKilo(capacity));
		    igTableSetColumnIndex(3);
		    igText("%.1f %%", (offset / (float)capacity) * 100.0f);
		}*/
		...
		/*{
			...
			igTableSetColumnIndex(1);
			igText("%.2f MB", ByteToMega(used));
			igTableSetColumnIndex(2);
			igText("%.2f MB", ByteToMega(leep::kMaxAlloc));
			igTableSetColumnIndex(3);
			igText("%.1f %%", (used / (float)leep::kMaxAlloc) * 100.0f);
		}*/
