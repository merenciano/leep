...
static leep::String selected_entity = "";
...
static void THE_UIToolsInfoWindow(bool *show)
{
	...
	ImGui::Text("Frame time: %f", leep::GM.delta_time() * 1000.0f);
	ImGui::Text("FPS: %f", 1.0f / leep::GM.delta_time());
	ImGui::Text("Init time (ms): %f", leep::GM.tools_data.init_time_ms);
	ImGui::Text("Logic average (ms): %f", leep::GM.tools_data.logic_average_ms);
	ImGui::Text("Render average (ms): %f", leep::GM.tools_data.render_average_ms);
	ImGui::PopItemWidth();
	ImGui::End();
}
...
static void THE_UIToolsEntityInspector()
{
	...
	auto it = leep::GM.scene().entities_->begin();
	while(it != leep::GM.scene().entities_->end()) {
		...
		case leep::EntityType::NONE:
			header_name = "";
			break;

		case leep::EntityType::RENDERABLE:
			header_name = "Renderable";
			break;

		case leep::EntityType::FALLING_CUBE:
			header_name = "InfiniteFalling";
			break;
		}

		if (ImGui::CollapsingHeader(header_name)) {
			for (u32 i = 0; i < it->second.blocks_.size(); ++i) {
				leep::String tree_name = "Chunk " + leep::ToString(i);
				if (ImGui::TreeNode(tree_name.c_str())) {
					...
					if (ImGui::BeginTable("##table1", 2, flags, size)) {
						...
						for (s32 j = 0; j < it->second.blocks_[i]->last_; ++j) {
							ImGui::TableNextRow();
							ImGui::TableSetColumnIndex(0);
							ImGui::Text("%d", j);
							ImGui::TableSetColumnIndex(1);
							//ImGui::Text("%s", Entity(j, it->first).name().c_str());
							if (ImGui::Button(leep::Entity(j + i * leep::kEntitiesPerChunk, it->first).name().c_str())) {
								show_components  = true;
								selected_entity = leep::Entity(j + i * leep::kEntitiesPerChunk, it->first).name();
							}
						}
						ImGui::EndTable();
					}
...
static void THE_UIToolsComponentInspector()
{
	...
	snprintf(window_title, 64, "Components of %s", selected_entity.c_str());
	...
	*/
	leep::Entity e = leep::Entity::GetEntity(selected_entity);
	if (e.hasComponent<leep::LTransform>()) {

	}

	if (e.hasComponent<leep::Drawable>()) {
		leep::Drawable& dw = e.getComponent<leep::Drawable>();
...
void THE_UIToolsCalcLogicAverage(float frame)
{
	...
	if (i == 32) {
		...
		leep::GM.tools_data.logic_average_ms = avg / 32.0f;
	}
}
...
void THE_UIToolsCalcRenderAverage(float frame)
{
	...
	if (i == 64) {
		...
		leep::GM.tools_data.render_average_ms = avg / 32.0f;
	}
}
